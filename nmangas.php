<?php
  ...
  $queryReg = "SELECT * FROM noticia";
	...
	//Consulta SQL
	$consultavistas ="SELECT * FROM	noticia WHERE tipo='0' ORDER BY id DESC LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar;
	$consulta = $conexion->query($consultavistas);
  ?>
  <div class="box box-footer">
    <div class="box-header">
      <center><h3>Noticias del Mundo del Manga</h3></center>
    </div>
    <?php
    ...
    <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <center>
              <h3 style="color:black;">Publicar Noticia<small> Mangas</small></h3>
            </center>
          </div>
          <div class="modal-body"><!--#Body modal comentar-->
            ...
            if (isset($_POST['btnpublicarn'])) {
              ...
              $querypublicarn = "INSERT INTO noticia (titulo,fecha,prev,noticia,tipo,id_usuario) VALUES ('$titulon',now(),'$prevn','$notician',0,'$yo')";
              $resultpublicarn = $conexion->query($querypublicarn);

              if ($resultpublicarn) { ?>
                <script type="text/javascript">
                  location.href = 'noticias.php?noticia=nmangas';
                </script>
              <?php
              }
            }
    ...
    <nav>
        <ul class="pagination">
          ...
          <?php
            ...
            <li><a href="noticias.php?pag=<?php echo $anteriornav ?>&noticia=nmangas"><i class="glyphicon glyphicon-chevron-left"></i> Anterior</a></li>
          <?php
          }?>
          <li><a href="noticias.php?noticia=nmangas"><i class="glyphicon glyphicon-home"></i> Inicio</a></li>
          <?php
            if ($IncrimentNum == 0) { ?>
              <li class="disabled"><a class="disable">Siguiente <i class="glyphicon glyphicon-chevron-right"></i></a></li>
            <?php
          }else{ ?>
            <li><a href="noticias.php?pag=<?php echo $IncrimentNum ?>&noticia=nmangas">Siguiente <i class="glyphicon glyphicon-chevron-right"></i></a></li>
          <?php
          }
          ?>
        </ul>
